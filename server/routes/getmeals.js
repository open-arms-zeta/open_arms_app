...
router.get('/mealExtras', function(req,res){
    ...
    pg.connect(connectionString, function (err, client) {
        var query = client.query("SELECT meals.entree, meals.side_1, meals.side_2, meals.status, categories.category_name, allergens.allergen_name, allergenspecific.specific_name\
        FROM meals\
...
router.get('/searchMeal', function(req,res){
    ...
    pg.connect(connectionString, function (err, client) {
        var query = client.query("SELECT meals.entree, meals.side_1, meals.side_2, categories.category_name, allergens.allergen_name, allergens.allergen_specific\
       FROM meals\
       JOIN meal_category ON meals.meal_id = meal_category.meal_id\
       JOIN categories ON categories.category_id = meal_category.category_id\
       JOIN meal_allergen_allergenspecific ON meal_allergen_allergenspecific.meal_id = meals.meal_id\
